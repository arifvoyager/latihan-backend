                        ...
                        <div class="example-wrap">
                            <div class="example">
                                <div class="btn-group hidden-xs" id="exampleTableEventsToolbar" role="group">
                                    <button type="button" class="btn btn-outline btn-default" data-toggle="tooltip" data-original-title="Add New" onclick="javascript:location.href='<?php echo base_url('cms/gallery/addnew'); ?>'">
                                ...
                                <table id="loadDataTables" data-mobile-responsive="true">
                                    <thead>
                                        <tr>
                                            <th data-field="taxonomy_lock_code" data-column-id="id" data-identifier="true" data-visible="false" data-filterable="true">ID</th>
                                            <th data-field="state" data-checkbox="true"></th>
                                            <th data-field="taxonomy_value">GALLERY TYPE</th>
                                            <th data-field="taxonomy_name">GALLERY CATEGORY</th>
...
<script type="text/javascript">
    ...
    $("#loadDataTables").on("selected.rs.jquery.bootgrid", function(e, rows) {
        for (var i = 0; i < rows.length; i++) {
            rowIds.push(rows[i].taxonomy_lock_code);
        }
    });
    ...
    $("#loadDataTables").on("deselected.rs.jquery.bootgrid", function(e, rows) {
        for (var i = 0; i < rows.length; i++) {
            var itemtoRemove = rows[i].taxonomy_lock_code;
    ...
    $(document).ready(function($) {
        ...
        (function() {
            $('#loadDataTables').bootstrapTable({
                url: "<?php echo base_url("cms/gallery/getData"); ?>",
                search: true,
                pagination: true,
                showRefresh: true,
                showToggle: false,
    ...
    function operateFormatter(value, row, index) {
        if (row['taxonomy_value'] == 'image') {
            return [
                    '<button type="button" class="btn btn-xs btn-icon btn-success btn-outline btn-list" title="List Data"><i class="icon wb-list" aria-hidden="true"></i></button> ',
            ...
            return [
                    '<button type="button" class="btn btn-xs btn-icon btn-success btn-outline btn-image" title="Video Link"><i class="icon wb-link" aria-hidden="true"></i></button> ',
                    '<button type="button" class="btn btn-xs btn-icon btn-primary btn-outline btn-edit" title="Edit Data"><i class="icon wb-pencil" aria-hidden="true"></i></button> ',
                    '<button type="button" class="btn btn-xs btn-icon btn-danger btn-outline btn-delete" title="Delete Data"><i class="icon fa-trash" aria-hidden="true"></i></button> '
            ].join('');
        }
    ...
    window.operateEvents = {
        'click .btn-edit': function (e, value, row, index) {
            window.location = BASE_URL + 'cms/gallery/editdata/' + row['taxonomy_lock_code'];
        },
        'click .btn-list': function (e, value, row, index) {
            window.location = BASE_URL + 'cms/gallery/loads/' + row['taxonomy_lock_code'];
        },
        'click .btn-image': function (e, value, row, index) {
            window.location = BASE_URL + 'cms/gallery/link/' + row['taxonomy_lock_code'];
        },
                ...
                buttons: {
                    ...
                    success: {
                        ...
                        callback: function () {
                            ...
                            $.ajax({
                                url : BASE_URL + 'cms/gallery/trash/',
                                type: 'POST',
                                data: {
                                    LOCK_CODE : row['taxonomy_lock_code']
                                }, 
                                success: function (response) {
                                    //console.log(response);
                                    window.location = BASE_URL + 'cms/gallery'
                                }
                            });
                ...
                buttons: {
                    ...
                    success: {
                        ...
                        callback: function () {
                            ...
                            $.ajax({
                                url : BASE_URL + 'cms/gallery/trashSelected/',
                                type: 'POST',
