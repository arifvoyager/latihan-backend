                        ...
                        <div class="example-wrap">
                            <div class="example">
                                ...
                                <table id="loadDataTables" data-mobile-responsive="true">
                                    <thead>
                                        <tr>
                                            <th data-field="general_lock_code" data-column-id="id" data-identifier="true" data-visible="false" data-filterable="true">ID</th>
                                            <th data-field="state" data-checkbox="true"></th>
                                            <th data-field="general_name">General Name</th>
                                            <th data-field="general_caption" class="col-md-5">General Value</th>
...
<script type="text/javascript">
    ...
    $("#loadDataTables").on("selected.rs.jquery.bootgrid", function(e, rows) {
        for (var i = 0; i < rows.length; i++) {
            rowIds.push(rows[i].general_lock_code);
        }
    });
    ...
    $("#loadDataTables").on("deselected.rs.jquery.bootgrid", function(e, rows) {
        for (var i = 0; i < rows.length; i++) {
            var itemtoRemove = rows[i].general_lock_code;
    ...
    $(document).ready(function($) {
        ...
        (function() {
            $('#loadDataTables').bootstrapTable({
                url: "<?php echo base_url("cms/general/getData"); ?>",
                search: true,
                pagination: true,
                showRefresh: true,
                showToggle: false,
                ...
                rowIds.push(row.general_lock_code);
                
            }).on('uncheck.bs.table', function (e, row) {
                //$result.text('Event: uncheck.bs.table, data: ' + JSON.stringify(row));
                var itemtoRemove = row.general_lock_code;
                rowIds.splice($.inArray(itemtoRemove, rowIds),1);
                ...
                for (var i = 0; i < row.length; i++) {
                    rowIds.push(row[i].general_lock_code);
                }
                
            }).on('uncheck-all.bs.table', function (e, row) {
                //$result.text('Event: Uncheck ALL');
                for (var i = 0; i < row.length; i++) {
                    var itemtoRemove = row[i].general_lock_code;
                    rowIds.splice($.inArray(itemtoRemove, rowIds),1);
                }
            });
    ...
    window.operateEvents = {
        'click .btn-edit': function (e, value, row, index) {
            window.location = BASE_URL + 'cms/general/editdata/' + row['general_lock_code'];
        }
    }
                ...
                buttons: {
                    ...
                    success: {
                        ...
                        callback: function () {
                            ...
                            $.ajax({
                                url : BASE_URL + 'cms/general/trashSelected/',
                                type: 'POST',
                                data: {data : jsonString}, 
                                success: function (response) {
                                    console.log(response);
                                    window.location = BASE_URL + 'cms/general'
                                }
                            });
                        }
                    }
