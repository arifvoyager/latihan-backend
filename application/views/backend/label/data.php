...
<div class="page animsition">
    <div class="page-header">
        ...
        <div class="page-header-actions">
            <button type="button" class="btn btn-sm btn-icon btn-inverse btn-round" data-toggle="tooltip" data-original-title="Add New" onclick="javascript:location.href='<?php echo base_url('jpanel/label/addnew'); ?>'">
                        ...
                        <div class="example-wrap">
                            <div class="example">
                                <div class="btn-group hidden-xs" id="exampleTableEventsToolbar" role="group">
                                    <button type="button" class="btn btn-outline btn-default" data-toggle="tooltip" data-original-title="Add New" onclick="javascript:location.href='<?php echo base_url('jpanel/label/addnew'); ?>'">
                                ...
                                <table id="loadDataTables" data-mobile-responsive="true">
                                    <thead>
                                        <tr>
                                            <th data-field="label_lock_code" data-column-id="id" data-identifier="true" data-visible="false" data-filterable="true">ID</th>
                                            <th data-field="state" data-checkbox="true"></th>
                                            <th data-field="label_flag">FLAG</th>
                                            <th data-field="label_caption">LABEL</th>
...
<script type="text/javascript">
    ...
    $("#loadDataTables").on("selected.rs.jquery.bootgrid", function(e, rows) {
        for (var i = 0; i < rows.length; i++) {
            rowIds.push(rows[i].label_lock_code);
        }
    });
    ...
    $("#loadDataTables").on("deselected.rs.jquery.bootgrid", function(e, rows) {
        for (var i = 0; i < rows.length; i++) {
            var itemtoRemove = rows[i].label_lock_code;
    ...
    $(document).ready(function($) {
        ...
        (function() {
            $('#loadDataTables').bootstrapTable({
                url: "<?php echo base_url("jpanel/label/getData"); ?>",
                search: true,
                pagination: true,
                showRefresh: true,
                showToggle: false,
                ...
                rowIds.push(row.label_lock_code);
                
            }).on('uncheck.bs.table', function (e, row) {
                //$result.text('Event: uncheck.bs.table, data: ' + JSON.stringify(row));
                var itemtoRemove = row.label_lock_code;
                rowIds.splice($.inArray(itemtoRemove, rowIds),1);
                ...
                for (var i = 0; i < row.length; i++) {
                    rowIds.push(row[i].label_lock_code);
                }
                
            }).on('uncheck-all.bs.table', function (e, row) {
                //$result.text('Event: Uncheck ALL');
                for (var i = 0; i < row.length; i++) {
                    var itemtoRemove = row[i].label_lock_code;
                    rowIds.splice($.inArray(itemtoRemove, rowIds),1);
                }
            });
    ...
    window.operateEvents = {
        'click .btn-edit': function (e, value, row, index) {
            window.location = BASE_URL + 'jpanel/label/editdata/' + row['label_lock_code'];
        },
                ...
                buttons: {
                    ...
                    success: {
                        ...
                        callback: function () {
                            ...
                            $.ajax({
                                url : BASE_URL + 'jpanel/label/trash/',
                                type: 'POST',
                                data: {
                                    LOCK_CODE : row['label_lock_code']
                                }, 
                                success: function (response) {
                                    //console.log(response);
                                    window.location = BASE_URL + 'jpanel/label'
                                }
                            });
                ...
                buttons: {
                    ...
                    success: {
                        ...
                        callback: function () {
                            ...
                            $.ajax({
                                url : BASE_URL + 'jpanel/label/trashSelected/',
                                type: 'POST',
