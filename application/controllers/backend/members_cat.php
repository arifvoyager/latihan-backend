...
class Members_cat extends CI_Controller {
	...
	public function index() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $data['STR_PAGES']       = 'PAGE_MEMBERS_CAT';
            $data['STR_MENU']        = 'members';
            $data['TITLE_PAGE']      = 'Members';
            $data['content']         = 'members/categories/data.php';
    ...
    public function getData() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $GET_LANG = $this->db->query("SELECT * FROM cd_taxonomy A WHERE taxonomy_flags = 'members_cat' AND taxonomy_lang = '".GET_DEFAULT_LANG()."' GROUP BY taxonomy_lock_code ORDER BY taxonomy_value DESC");
            $strArray = array();
	...
	public function addnew() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $data['content']         = 'members/categories/addnew.php';
            $this->load->view('backend/index', $data);
    ...
    public function saved() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $data['taxonomy_flags']         = 'members_cat';
            ...
            //- REDIRECT
            redirect(base_url('cms/members_cat'));
	...
	public function editdata($LOCK_CODE) {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $data['content']         = 'members/categories/edit.php';
