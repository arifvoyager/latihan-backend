...
class News extends CI_Controller {
	...
	public function index() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $data['STR_PAGES']       = 'PAGE_NEWS';
            $data['STR_MENU']        = 'news';
            $data['TITLE_PAGE']      = 'NEWS';
            $data['content']         = 'news/data.php';
    ...
    public function getData() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $GET_LANG = $this->db->query("SELECT *, (SELECT taxonomy_value FROM cd_taxonomy B WHERE A.posts_category = B.taxonomy_lock_code AND B.taxonomy_lang = '".GET_DEFAULT_LANG()."') as news_category FROM cd_posts A WHERE posts_type = 'news' AND posts_lang = '".GET_DEFAULT_LANG()."' GROUP BY posts_lock_code ORDER BY posts_date DESC");
            $strArray = array();
	...
	public function addnew() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $data['STR_PAGES']       = 'PAGE_NEWS_ADDNEW';
            $data['STR_MENU']        = 'news';
            $data['TITLE_PAGE']      = 'Add New';
            $data['content']         = 'news/addnew.php';
            $this->load->view('backend/index', $data);
    ...
    public function saved() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $data['posts_type']             = 'news';
            $data['posts_lock_code']        = getRandom(15, 15);
            $data['posts_locked']           = 'N';
            
            //UPLOAD IMAGES
            if ($_FILES['posts_images']['error'] == 0) {
                $upload_image                   = upload_image('posts_images', 750, 320);
