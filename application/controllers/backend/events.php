...
class Events extends CI_Controller {
	...
	public function index() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $data['STR_PAGES']       = 'PAGE_EVENTS';
            $data['STR_MENU']        = 'events';
            $data['TITLE_PAGE']      = 'Events';
            $data['content']         = 'events/data.php';
    ...
    public function getData() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $GET_LANG = $this->db->query("SELECT * FROM cd_posts A WHERE posts_type = 'events' AND posts_lang = '".GET_DEFAULT_LANG()."' GROUP BY posts_lock_code ORDER BY posts_date ASC");
            $strArray = array();
			foreach($GET_LANG->result() as $strObj) {
                if ($strObj->posts_start_date) {
                    $strObj->posts_start_date = date("d M Y", strtotime($strObj->posts_start_date)); 
                }
                if ($strObj->posts_end_date) {
                    $strObj->posts_end_date = date("d M Y", strtotime($strObj->posts_end_date)); 
                }
				$strArray[] = $strObj;
			}
	...
	public function addnew() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $data['STR_PAGES']       = 'PAGE_EVENTS_ADDNEW';
            $data['STR_MENU']        = 'events';
            $data['TITLE_PAGE']      = 'Add New';
            $data['content']         = 'events/addnew.php';
            $this->load->view('backend/index', $data);
    ...
    public function saved() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $data['posts_type']             = 'events';
            $data['posts_lock_code']        = getRandom(15, 15);
            $data['posts_locked']           = 'N';
            
            //UPLOAD IMAGES
            if ($_FILES['posts_images']['error'] == 0) {
                $upload_image                   = upload_image('posts_images', GET_GENERAL('image_events_width'), GET_GENERAL('image_events_height'));
            ...
            foreach ($GET_LANG->result() as $LANG):
                ...
                endforeach;
                $data['posts_start_date']       = date("Y-m-d H:i:s", strtotime($this->input->post('posts_start_date')));                
                $data['posts_end_date']         = date("Y-m-d 23:59:59", strtotime($this->input->post('posts_end_date')));                
