...
class News_cat extends CI_Controller {
	...
	public function index() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $data['STR_PAGES']       = 'PAGE_NEWS_CATEGORY';
            $data['STR_MENU']        = 'news';
            $data['TITLE_PAGE']      = 'News Category';
            $data['content']         = 'news/categories/data.php';
    ...
    public function getData() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $GET_LANG = $this->db->query("SELECT * FROM cd_taxonomy A WHERE taxonomy_flags = 'news_cat' AND taxonomy_lang = '".GET_DEFAULT_LANG()."' GROUP BY taxonomy_lock_code ORDER BY taxonomy_value DESC");
            $strArray = array();
	...
	public function addnew() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $data['content']         = 'news/categories/addnew.php';
            $this->load->view('backend/index', $data);
    ...
    public function saved() {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            //- REDIRECT
            redirect(base_url('cms/news_cat'));
	...
	public function editdata($LOCK_CODE) {
        if ($this->session->userdata('sess_cpanelx') == TRUE) {
            ...
            $data['STR_PAGES']       = 'PAGE_NEWS_CATEGORY';
            $data['STR_MENU']        = 'news';
            $data['TITLE_PAGE']      = 'Edit News Category';
            $data['LOCK_CODE']       = $LOCK_CODE;
            $data['content']         = 'news/categories/edit.php';
